...
FastAPIの待受開始後、Chromeの`Default`プロファイルで`http://127.0.0.1:8000/`が開きます。Chromeを閉じると、待機中・実行中ジョブがなければアプリも終了します。ジョブがある場合は最後まで処理し、完了後も画面が再接続されなければ終了します。Chromeを起動できなかった場合はコンソールログを確認し、必要なら起動後10秒以内に同URLを手動で開いてください。

バックアップ対象は、各プロジェクトのルートフォルダ全体と`%LOCALAPPDATA%\LoRAMaker`です。前者には原本・工程出力・プロジェクト設定、後者には個人設定とジョブ履歴があります。復旧時は両方を元の場所へ戻し、`app_master.json`および外部ツールのパス設定を確認してください。

障害時は、起動したPowerShellのログ、画面のジョブログ、`%LOCALAPPDATA%\LoRAMaker\jobs.sqlite3`の順に確認します。ComfyUI接続、sd-scripts、ffmpeg、BandiViewまたは保存先で失敗した場合は、画面に表示された対象パスと個人設定の外部ツールパスが現在も存在するか確認してください。異常終了時に実行中だったジョブは再起動後に失敗として記録されるため、入力と既存出力を確認してから再実行します。
